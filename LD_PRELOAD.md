...
Некоторые разработчики используют memset() для затирания чувствительных данных в памяти. Но это — **грубая ошибка**.
...
- Ты перехватываешь не сам syscall, а его удобную обёртку из библиотеки. Этого достаточно, чтобы атаковать 99% программ, потому что все используют эти обёртки.
...
## ♦️ Продолжение будет в тгк - https://t.me/dima853_code

**все исходники (код, ссылки и тд), детальные объяснения, и все остальное в закрытом тг канале**
